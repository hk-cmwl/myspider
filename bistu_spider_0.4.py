...
def spider(starturl,starturldescription,urlfilter,titlefilter,localfilter):
    ...
    while newdict:
        ...
        if not x:#此url未被处理过
            ...
            tmp=tmp[0].replace('\n',' ')
            while tmp:
                t1=re.match(r'(.*?)<(.*?)>(.*)',tmp)#取内容
                try:
                    ct1=t1.group(1).decode('UTF-8').encode('gbk')
                except:
                    try:
                        ct1=t1.group(1)
                    except:
                ...
                if ct1:
                    onlyct=onlyct+' '+ct1
                tmp=t1.group(3)
            print '正在处理链接：'+key+'   网页简要说明：'+value+'  网页标题：'+title+'    网页内容：'+onlyct
...
result=spider(starturl,'北京信息科技大学',urlfilter,titlefilter,localfilter)         
